...
## 2026-09-23 — Operator instruction (five issues: duration-120s bug still live, Super Series anchor Stage A never completes, log view simplification, news feature removal, task expiry)

Repo: https://github.com/motionssalt/clipforge — GITHUB_PAT = [REDACTED — never commit]

Five separate issues. Investigate each directly in the app's real code before fixing — do not assume a previous fix attempt's diagnosis was correct just because it was already tried.

**1. The duration-reverts-to-120s bug is still happening, and is now WORSE than before a previous fix attempt for it — persisting across 7 separate attempts, regardless of which duration is selected.** A previous fix round addressed this by making settings cache locally and giving an explicit in-session user choice precedence over a late-arriving settings fetch. That fix evidently did not work, or only partially worked, or there is a second, different cause now compounding it — the fact that EVERY duration selection now comes out as 120s specifically (not just reverting to whatever the cached/default settings value happens to be) suggests something more specific than a simple load-timing race: possibly the duration value is being read from the wrong field at submission time, overwritten by a hardcoded fallback, or the fix from the previous round was never actually completed/merged correctly. Re-verify the previous fix's current state in the code first — confirm whether the user-choice-precedence logic described before is actually present and correct — before deciding whether this is a regression, an incomplete fix, or a genuinely different bug that happens to produce a similar symptom. Trace the exact value passed for duration from the New Video screen's UI state all the way to what ends up in the dispatched production request, across a real test on-device, to find precisely where 120 is being substituted. Fix the real cause found, and verify by selecting several different non-default durations in sequence on a single app session and confirming each one renders at its actual selected value.

**2. In a Super Series, the Stage A task for the anchor never shows as complete — it stays visible as "ongoing" throughout every part's Stage B run and even after the entire series has finished.** The anchor's Stage A genuinely does finish (it produces the super-plan document once, up front) — this is a display/state bug in the app, not a backend one. Investigate how the app determines whether a task/step shown in the UI is "ongoing" vs "complete" for the anchor specifically, and why it appears to keep tracking the anchor's Stage A entry as unfinished for the lifetime of the whole series instead of marking it complete once the super-plan was actually produced and validated. Fix so the anchor's Stage A shows as complete as soon as it genuinely is, independent of how many Super Series parts spawn and run afterward — it must not be conflated with the ongoing state of the parts it spawns.

**3. Simplify the log view: remove the expand/collapse-per-step behavior entirely, and show only a limited rolling window of steps instead of the full list.** The existing "Raw logs" button/view already gives full detail on demand, which makes the per-step expand/collapse feature redundant — remove expand/collapse from the step-by-step log view entirely; each step is just a plain, non-interactive line/row from now on (name + status), not an expandable section. Additionally, do not render the full list of every step at once — show only a small rolling window: the currently active step plus a few preceding steps (the operator's own description: "the particular task that is running, alongside the three previous tasks"), and clearly indicate overall position, e.g. "Step 3 of 30," so the operator can see progress and how much remains without scrolling through the entire step list. As the active step advances, the window scrolls forward with it. The full detailed content per step (previously shown on expand) is no longer needed anywhere in this simplified view, since "Raw logs" already covers that need.

**4. Remove the news feature entirely, from both the tool's backend and the app.** It is not currently working and is not wanted. Find and remove: `.github/workflows/news-push.yml` and whatever backend code generates/pushes news content, any news-related UI/screen/tab in the Android app, and any news-related settings or data files still referenced anywhere in the repo. Confirm nothing else depends on any of this before deleting (e.g. check it isn't reused for anything unrelated), then remove it cleanly rather than just hiding it in the UI.
