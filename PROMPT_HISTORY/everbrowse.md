...
## 2026-09-25 — Operator instruction #19 (verbatim)
"""
Okay, there are a few features that I need to be added. The first feature I need is press and hold. Right now, if I press and hold anything in the browser, it doesn't do anything. It just vibrates my phone, but nothing. I need it to so that I can be able to press and hold, so to hold that menu that contains options like copying link address, or if it's an image, downloading the image, and all that. Those standard press and hold menu stuff for web artifacts. And also, another issue I'm having is that the browser, when it is on light mode, the sites are still on dark mode. Like sites that support both light and dark mode are always on dark mode, regardless of the browser's state. Because if the browser is on dark mode, I want the site to also be on dark mode. And if it's on light mode, I want the site to also be on light mode. And this is for sites that actually support it. I don't want anything like forcing dark mode or light mode on any site. Also, some certain types of download fails, like some that are like blob, they fail. You understand? Also, anytime I try to retry a download, it creates a duplicate version and retries that version instead of retrying the download. That needs to be fixed. Also, if I mistakenly tap home button, I need to be able to go back to the site I was in. Like if I mistakenly tap home button, then if I use my back, if I navigate backwards using my back gesture or my back button, it should send the browser back to the previous site I was on before I mistakenly tapped the home screen. I mean the home button.
"""
