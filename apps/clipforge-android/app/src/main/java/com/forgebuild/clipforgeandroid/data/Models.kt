...
object AgentPromptBuilder {
    fun build(status: TaskStatus, request: JSONObject?, owner: String, repo: String): String? {
        ...
        return "Open this GitHub release: " + releaseUrl + "\n" +
            ...
            "MANDATORY — validate before you deliver: this release also contains validator.py, which runs the EXACT SAME validation Stage B will run (it imports the real pipeline/plan/schema.py and pipeline/stage_b/series_reconcile.py — it is not a separate copy). After producing the file, run: python validator.py " + (if (isSuperSeries) "super-plan.json --super-plan" else "production.json") + " --job-id " + status.jobId + " from a checkout of the " + owner + "/" + repo + " repository (the folder that contains pipeline/). If it reports errors, fix them and re-run, repeating until it exits cleanly; if it auto-corrects series metadata in your file, keep those corrections. You MUST NOT deliver or attach the file until validator.py passes it cleanly — Stage B runs this exact code and will reject the file for the same reasons.\n" +
