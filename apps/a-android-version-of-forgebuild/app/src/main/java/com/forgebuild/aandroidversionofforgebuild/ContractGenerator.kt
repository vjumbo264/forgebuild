...
object ContractGenerator {
    ...
    private fun contractHead(kind: String): String = """
# ForgeBuild App Build Contract — $kind
...
""".trimIndent()
    ...
    fun promptNewApp(desc: String, slug: String): String {
...
2. Set identity inside apps/$slug/: namespace/applicationId in app/build.gradle.kts (derive a safe package, e.g. $safePkg), app_name in res/values/strings.xml.
...
CURRENT LATEST RELEASE = ${latest.ifEmpty { "(check the Releases API filtered by prefix $slug-)" }}
...
5. Release the next version via the release.yml workflow (app_path=apps/$slug); the tag auto-increments to $slug-v(N+1). NEVER overwrite or delete any prior release. Verify the new release + APK exist on the Releases API before completing.
6. Set apps/$slug/BUILD_STATE.json build_complete: true with current_task: none.
$RULES
""".trimIndent()
    }

    fun promptResume(slug: String): String {
        return """${contractHead("RESUME BUILD")}
APP_SLUG = $slug
APP_FOLDER = apps/$slug/ (inside $OWNER/$REPO)

INSTRUCTION:
Resume the interrupted build of $slug. Read apps/$slug/BUILD_STATE.json and PROMPT_HISTORY/$slug.md completely. Find the first task with status: "in_progress" or "pending". Complete it, update BUILD_STATE.json, commit + push, verify the push on origin/main, then move to the next task until the release is verified live and build_complete is true.
$RULES
""".trimIndent()
    }
}
